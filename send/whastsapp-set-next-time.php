...
if ($this->TemDisparo($pdo)) {
    $this->set2No($pdo);
    $this->chamarApiNotificacao($urlWhastapp);
} else {
...
{
    $sql  = "UPDATE Send SET NextWhatsapp = NOW() + INTERVAL 30 MINUTE";
...
function set2No($pdo)
{
    $sql  = "UPDATE Send SET flWhatsapp = 0, LastWhatsapp = NOW(), NextWhatsapp = NOW() + INTERVAL 30 MINUTE";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
}
...
{
    $sql  = "SELECT flWhatsapp from Send Limit 1";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    ...
    if ($resultado) {
        $flWhatsapp_variavel = $resultado['flWhatsapp'];

        return (bool) $flWhatsapp_variavel;
    }
...
function chamarApiNotificacao($urlWhastapp)
{
    $url = $urlWhastapp;
    $ch  = curl_init($url);
    ...
    if (curl_errno($ch)) {
        error_log('Erro ao chamar API de notificação: ' . curl_error($ch));
        curl_close($ch);
    ...
    if ($http_code >= 200 && $http_code < 300) {
        // Sucesso
        error_log('API de notificação chamada com sucesso. Status: ' . $http_code);
        return true;
    } else {
        // Erro na API (status 4xx ou 5xx)
        error_log('Falha na chamada da API de notificação. Status: ' . $http_code . ' Resposta: ' . $response);
        return false;
    }
}
